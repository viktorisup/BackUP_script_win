...
class MySMB:
    ...
    def __init__(self, username, password, server_ip, port, remote, domain, local_dir, smb_base_dir, smb_dir):
        self.backup_folder = 'Backup_' + os.path.basename(local_dir) + '_' + time.strftime("%Y-%m-%d")
    ...
    def delit_folder(self):
        self.samba.deleteDirectory(self.smb_base_dir, self.smb_dir)
